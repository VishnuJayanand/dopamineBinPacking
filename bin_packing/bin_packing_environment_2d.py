...
BIG_NEG_REWARD = -100
BIG_POS_REWARD = 10
HSIZE=10 #Bag capacity + 1...
VSIZE=100 #Time horizon/SIZE_PER_CELL
SIZE_PER_CELL=10#Or 100
...
#                  'item_probabilities': [0, 0, 0, 1/3, 0, 0, 0, 0, 2/3], #linear waste
        ...
        self.observation_space = spaces.Box(low=0., high=255., shape=(HSIZE,VSIZE), dtype=np.uint32)
    ...
    def reset(self):
        ...
        initial_state = np.zeros((HSIZE,VSIZE), dtype=np.uint32)
        for i in range(0,self.item_size):
            initial_state[HSIZE-1][i]=255
    ...
    def step(self, action):
        ...
        state = np.zeros((HSIZE,VSIZE), dtype=np.uint32)
        for i in range(0,self.item_size):
            state[HSIZE-1][i]=255
        for i in range(0,len(self.num_bins_levels)):
            if self.num_bins_levels[i]>0:
                buckets_used=math.ceil(self.num_bins_levels[i]/SIZE_PER_CELL)
                for j in range(0,buckets_used-1):
                    state[i][j]=255
                state[i][buckets_used]=255*((SIZE_PER_CELL-(self.num_bins_levels[i]%SIZE_PER_CELL))/SIZE_PER_CELL)
    ...
    def __insert_item(self, action):
        if action == 0:  # new bag
            ...
            if action + self.item_size == self.bag_capacity:
                ...
                #print("Update bin levels near neighbor")
                self.num_bins_levels[action + self.item_size] += 1
            #print("Update bin levels near neighbor, reduce action by 1")
            self.num_bins_levels[action] -= 1
...
class BinPackingActionMaskGymEnvironment(BinPackingGymEnvironment):
    def __init__(self, env_config={}):
        super().__init__(env_config)
...
if __name__ == '__main__':
    env_config = {
        ...
        'item_probabilities': [0.7,0.3],#, 0, 0, 1 / 3, 0, 0, 0, 0, 2 / 3],  # linear waste
        # 'item_probabilities': [0.14, 0.10, 0.06, 0.13, 0.11, 0.13, 0.03, 0.11, 0.19], #bounded waste
    ...
    print(initial_state["real_obs"])
    done = False
    count = 0
    total_reward = 0
    while not done:        
        action = int(input("Act: "))#act#env.action_space.sample()
        print("Action: "+str(action))
        next_state, reward, done, _ = env.step(action)
        print(next_state["real_obs"])
        total_reward += reward
